...
export class UrlActivityRecord extends ActivityRecord {
    ...
    */
   public constructor(id_: string | undefined, email_: string, happenedAt_: Date, url_: string);
   ...
   streamOut(): string {
      ...
      return JSON.stringify({ id: this.id, email: this.email, happenedAt: this.happenedAt, url: this._url});
   }
   ...
   streamIn(stream: string): void {
      ...
      this.assign(new UrlActivityRecord (obj.id, obj.email, new Date(obj.happenedAt), obj.url));
   }
